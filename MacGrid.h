...
// For interpolating the x-component of velocity, the world input should have an offset of (0,0.5), so that makes it for the center of a grid cell (i,j) (i*dx, (j+0.5)*dy)
// For interpolating the y-component of velocity, the world input should have an offset of (0.5,0), so that makes it for the center of a grid cell (i,j) ((i+0.5)*dx, j*dy)
// For interpolating scalar values taken at the center of the cell (i,j), the world input should have an offset of (0.5,0.5), so that makes ((i+0.5)*dx, (j+0.5)*dy)
// (the last one is nothing more than getting the correct coordinates of the center of a cell)

 

class MacGrid : public GridInterface {
public:
    MacGrid(int size_x, int size_y, float length_x, float length_y, float ambient_temperature, float ambient_concentration);
    ~MacGrid();
    ...
    // Getters
    inline float temperature(int i, int j) const {
        return _temperature_front_buffer->value(i, j);
    };

    inline float temperatureBackBuffer(int i, int j) const {
        return _temperature_back_buffer->value(i, j);
    };

    inline float concentration(int i, int j) const {
        return _concentration_front_buffer->value(i, j);
    };

    inline float concentrationBackBuffer(int i, int j) const {
        return _concentration_back_buffer->value(i, j);
    };
    ...
    inline float velXHalfIndexed(int i, int j) const   {
        return _vel_x_front_buffer->value(i, j);
    };

    inline float velYHalfIndexed(int i, int j) const   {
        return _vel_y_front_buffer->value(i, j);
    };

    inline float velXBackBufferHalfIndexed(int i, int j) const   {
        return _vel_x_back_buffer->value(i, j);
    };
    
    inline float velYBackBufferHalfIndexed(int i, int j) const   {
    ...
    inline float temperatureInterpolated(float x, float y) const {
        ...
        float temperature =
            _temperature_front_buffer->valueInterpolated(x - 0.5 * _DELTA_X, y - 0.5 * _DELTA_Y);
        return temperature;
    };
    ...
    inline float concentrationInterpolated(float x, float y) const {
        // The subtraction on x and y is necessary due to valueInterpolated function working for the staggered grid
        float concentration =
            _concentration_front_buffer->valueInterpolated(x - 0.5 * _DELTA_X, y - 0.5 * _DELTA_Y);
    ...
    // Setters
    inline void setTemperature(int i, int j, float temperature) {
        (*_temperature_front_buffer)(i, j) = temperature;
    };
    ...
    inline void setVelXHalfIndexed(int i, int j, float vel_x) {
        (*_vel_x_front_buffer)(i, j) = vel_x;
    };

    inline void setVelYHalfIndexed(int i, int j, float vel_y) {
    ...
    inline void setVelXBackBufferHalfIndexed(int i, int j, float vel_x)   {
        (*_vel_x_back_buffer)(i, j) = vel_x;
    };

    inline void setVelYBackBufferHalfIndexed(int i, int j, float vel_y)   {
        (*_vel_y_back_buffer)(i, j) = vel_y;
    };
