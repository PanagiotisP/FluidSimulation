...
public:
    GridInterface(int size_x, int size_y, float delta_x = 1, float delta_y = 1):
     _SIZE_X(size_x), _SIZE_Y(size_y), _DELTA_X(delta_x), _DELTA_Y(delta_y) {};
    ~GridInterface() {};
    ...
    inline bool indexIsValid(int i, int j) const { return i >= 0 && i < _SIZE_X && j >= 0 && j < _SIZE_Y; };
...
template <class T>
Grid<T>::Grid(int size_x, int size_y, float delta_x, float delta_y): GridInterface(size_x, size_y, delta_x, delta_y) {
    int n_elements = _SIZE_X * _SIZE_Y;
    data.resize(n_elements);
    for (int i = 0; i < n_elements; ++i) { data[i] = T(); }
}
...
template <class T>
inline Grid<T> &Grid<T>::operator=(const Grid<T> &a) {
    for (int j = 0; j < a.sizeX(); ++j) {
        for (int i = 0; i < a.sizeX(); ++i) { data[twoDToLinear(i, j)] = a.value(i, j); }
    }
    return *this;
};
